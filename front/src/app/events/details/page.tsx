...
import detalleDialog  from "@/components/details/details";

const traerEvento = async () => {
  try {
    const response = await axios.get("http://localhost:3001/api/events/65d78a84054d379e2e792a3d");
...
const detalle =  () => {
  ...
  try {
    const response =  axios.get("http://localhost:3001/api/events/").then((res) => {
      for (let i = 0; i < res.data.length; i++) {
        eventos.push(res.data[i]);
      }
    }
    );
