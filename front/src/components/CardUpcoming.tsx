...
export default function CardUpcoming({event, index}: {event: Ieventos, index: number}){
  const {fecha, mes, hora} = dateFormat(event.startDate)
    ...
    >
      ...
      >
        ...
        <div className={` h-1/2 w-full  ${index === 0 ? "flex justify-end p-8":"flex  items-center justify-center p-8"} "`}>
          <div className="w-1/2 flex flex-col ">
            <p className="block text-3xl text-center text-white font-bold">{mes}</p>
            <p className="block text-7xl text-center text-white font-bold">{fecha}</p>
            </div>
        </div>
      </CardContent>
      <CardFooter className={`flex  justify-start  py-2 rounded-b-xl`}>
        ...
        <div className="flex flex-col items-center ">
          ...
          <CardDescription className="text-sm">{hora}</CardDescription>
        </div>
      </CardFooter>
    </Card>
    )
